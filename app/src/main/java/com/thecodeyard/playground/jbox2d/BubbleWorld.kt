...
class BubbleWorld(private val listener: Listener) {
    ...
    /**
     * Start the simulation. If the simulations was already started before, then a subsequent call to this method will simply stop it and restart it.
     */
    fun startSimulation() {
        stopSimulation()
    ...
    private fun update() {
        ...
        while (body != null) {
            ...
            if (bubble is Bubble) {
                // NOTE: In the 2D world (x,y) is the center of a body whereas in the Android world (x,y) is the top left corner of a view.
